...
export default function BidderMarketplaceNavbar({ user }) {
  ...
  const userInitial = user?.full_name?.charAt(0)?.toUpperCase() || user?.email?.charAt(0)?.toUpperCase() || 'B'
        ...
        <div className="flex items-center justify-between h-16">
            ...
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === 'Enter' && event.target.value.trim()) {
                  navigate(`/auctions?q=${encodeURIComponent(event.target.value.trim())}`)
                }
              }}
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex flex-col text-right">
              ...
              <span className="text-xs text-gray-500 capitalize">{user?.role || 'bidder'}</span>
            </div>
