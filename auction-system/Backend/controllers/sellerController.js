...
export const createProduct = async (req, res) => {
  try {
    ...
    if (!startPriceNumber || Number.isNaN(startPriceNumber) || startPriceNumber < 0) {
      return res.status(400).json({ success: false, message: "Giá khởi điểm không hợp lệ." });
    }

    if (!stepPriceNumber || Number.isNaN(stepPriceNumber) || stepPriceNumber <= 0) {
...
export const updateProduct = async (req, res) => {
  try {
    const { id } = req.params;
    const seller_id = req.user.id;
    const { name, category_id, starting_price, step_price, buy_now_price, end_time, images, allow_unrated_bidders, auto_extend, auto_extend_minutes, auto_extend_threshold, append_description } = req.body;

    const { data: product, error: checkError } = await supabase.from("products").select("*").eq("id", id).eq("seller_id", seller_id).single();
    ...
    const hasMutableChanges = Boolean(name || category_id || starting_price !== undefined || step_price !== undefined || buy_now_price !== undefined || end_time || Array.isArray(images) || allow_unrated_bidders !== undefined || auto_extend !== undefined || auto_extend_minutes !== undefined || auto_extend_threshold !== undefined);
    ...
    res.json({
      success: true,
      message: append_description ? "Đã bổ sung mô tả mới thành công. Mô tả gốc được giữ nguyên." : "Cập nhật sản phẩm thành công.",
      data: updated,
    });
  } catch (error) {
...
export const answerBidderQuestion = async (req, res) => {
  try {
    ...
    const { data: question, error: questionError } = await supabase.from("questions").select("id, product_id").eq("id", questionId).single();

    if (questionError || !question) {
      return res.status(404).json({ success: false, message: "Không tìm thấy câu hỏi." });
    }

    const { data: product, error: productError } = await supabase.from("products").select("id, seller_id").eq("id", question.product_id).single();
    ...
    const { data: updated, error: updateError } = await supabase.from("questions").update(updates).eq("id", questionId).select("id, product_id, asker_id, question, answer, answered_at, created_at, profiles:asker_id ( full_name )").single();
    ...
    try {
      ...
      const { data: fullProduct } = await supabase.from("products").select("id, name, thumbnail_url").eq("id", question.product_id).single();

      const { data: seller } = await supabase.from("profiles").select("id, email, full_name").eq("id", sellerId).single();
...
export const getProductBids = async (req, res) => {
  try {
    ...
    // Kiểm tra quyền sở hữu
    const { data: product } = await supabase.from("products").select("id").eq("id", id).eq("seller_id", seller_id).single();
...
export const rejectBid = async (req, res) => {
  try {
    ...
    const { data: product, error: productError } = await supabase.from("products").select("id, seller_id, status, starting_price, step_price").eq("id", productId).single();
    ...
    const { data: bid, error: bidError } = await supabase.from("bids").select("id, product_id, bidder_id, is_rejected").eq("id", bidId).eq("product_id", productId).single();
    ...
    const { data: existingReject } = await supabase.from("rejected_bidders").select("id").eq("product_id", productId).eq("bidder_id", bidderToRejectId).maybeSingle();

    if (!existingReject) {
      const { error: rejectInsertError } = await supabase.from("rejected_bidders").insert({
    ...
    const { count } = await supabase.from("bids").select("*", { count: "exact", head: true }).eq("product_id", productId).eq("is_rejected", false);
    ...
    try {
      const { data: bidder } = await supabase.from("profiles").select("id, email, full_name").eq("id", bidderToRejectId).single();

      const { data: fullProduct } = await supabase.from("products").select("id, name, thumbnail_url").eq("id", productId).single();
...
export const getWinnerSummary = async (req, res) => {
  try {
    ...
    const { data: product, error: productError } = await supabase.from("products").select("id, seller_id, name, status, final_price, current_price, starting_price, winner_id, end_time, buy_now_price").eq("id", id).single();
    ...
    const { data: winner } = await supabase.from("profiles").select("id, full_name, email, phone, rating_positive, rating_negative").eq("id", product.winner_id).single();

    const { data: order } = await supabase.from("orders").select("id, status, payment_proof_url, shipping_address, payment_confirmed_at, created_at, updated_at, cancelled_at, cancellation_reason").eq("product_id", id).maybeSingle();

    const { data: ratingHistory } = await supabase.from("ratings").select("id, rating, comment, created_at").eq("product_id", id).eq("from_user_id", sellerId).eq("to_user_id", product.winner_id).order("created_at", { ascending: false });
...
export const rateWinner = async (req, res) => {
  try {
    ...
    const { data: product, error: productError } = await supabase.from("products").select("id, seller_id, status, winner_id").eq("id", id).single();
    ...
    const { data: existingRatings } = await supabase.from("ratings").select("id, rating").eq("product_id", id).eq("from_user_id", sellerId).eq("to_user_id", product.winner_id);
    ...
    const [posRes, negRes] = await Promise.all([supabase.from("ratings").select("id", { count: "exact", head: true }).eq("to_user_id", userIdToUpdate).eq("rating", "positive"), supabase.from("ratings").select("id", { count: "exact", head: true }).eq("to_user_id", userIdToUpdate).eq("rating", "negative")]);
...
export const cancelWinnerTransaction = async (req, res) => {
  try {
    ...
    const { data: product, error: productError } = await supabase.from("products").select("id, seller_id, status, winner_id, starting_price").eq("id", id).single();
...
export const reopenAuction = async (req, res) => {
  try {
    ...
    const { data: product, error: productError } = await supabase.from("products").select("id, seller_id, status, starting_price").eq("id", id).single();
    ...
    const { data: ratingHistory, error: ratingHistoryError } = await supabase.from("ratings").select("rating, comment").eq("product_id", id).eq("from_user_id", sellerId);

    if (ratingHistoryError) throw ratingHistoryError;

    const allowReopenByRating = Array.isArray(ratingHistory) && ratingHistory.length === 1 && ratingHistory[0].rating === "negative" && (ratingHistory[0].comment || "").trim().toLowerCase() === "người thắng không thanh toán";
...
export const getSalesStats = async (req, res) => {
  try {
    ...
    // Tổng số sản phẩm
    const { count: totalProducts } = await supabase.from("products").select("id", { count: "exact" }).eq("seller_id", seller_id);

    // Sản phẩm đang active
    const { count: activeProducts } = await supabase.from("products").select("id", { count: "exact" }).eq("seller_id", seller_id).eq("status", "active");

    // Sản phẩm đã bán (sold)
    const { count: soldProducts } = await supabase.from("products").select("id", { count: "exact" }).eq("seller_id", seller_id).eq("status", "completed");

    // Tổng doanh thu (tổng current_price của sản phẩm sold)
    const { data: soldData } = await supabase.from("products").select("current_price").eq("seller_id", seller_id).eq("status", "completed");
...
export const getSellerProfile = async (req, res) => {
  try {
    ...
    const { data: profile, error } = await supabase.from("profiles").select("id, email, full_name, avatar_url, rating_positive, rating_negative, role").eq("id", sellerId).single();
...
export const updateSellerProfile = async (req, res) => {
  try {
    ...
    const { data: profile, error } = await supabase.from("profiles").update(updates).eq("id", sellerId).select("id, email, full_name, avatar_url, rating_positive, rating_negative, role").single();
...
export const uploadSellerAvatar = async (req, res) => {
  try {
    ...
    const { error } = await supabase.from("profiles").update({ avatar_url: publicUrl, updated_at: new Date().toISOString() }).eq("id", sellerId);
...
export const getBidRequests = async (req, res) => {
  try {
    ...
    // Check ownership
    const { data: product, error: productError } = await supabase.from("products").select("seller_id").eq("id", productId).single();
...
export const updateBidRequestStatus = async (req, res) => {
  try {
    ...
    // Update
    const { error: updateError } = await supabase.from("product_allowed_bidders").update({ status, updated_at: new Date().toISOString() }).eq("id", requestId);
...
export const getExpirationStatus = async (req, res) => {
  try {
    ...
    const { data: seller, error } = await supabase.from("profiles").select("id, role, seller_expired_at").eq("id", sellerId).single();
    ...
    const daysRemaining = expiredAt && !isExpired ? Math.ceil((expiredAt - now) / (1000 * 60 * 60 * 24)) : 0;
...
export const requestExtension = async (req, res) => {
  try {
    ...
    const { data: seller, error: roleError } = await supabase.from("profiles").select("role").eq("id", sellerId).single();
    ...
    const { data: existingRequest, error: checkError } = await supabase.from("upgrade_requests").select("*").eq("user_id", sellerId).eq("status", "pending").is("reviewed_at", null).order("created_at", { ascending: false }).limit(1).single();
