import express from 'express'
import passport from '../config/passport.js'
import jwt from 'jsonwebtoken'
import { supabase } from '../config/supabase.js'
import { register, login, refresh, logout, getProfile, resendVerification, verifyOTPCode, getAccountType, changePassword, createPassword, forgotPassword, verifyResetOTP, resetPassword } from '../controllers/authController.js'
import { authenticate } from '../middleware/auth.js'

const router = express.Router()

const JWT_SECRET = process.env.JWT_SECRET || 'your-super-secret-jwt-key-change-this-in-production'

// ═══════════════════════════════════════════════════════════════════════════
// TRADITIONAL AUTH (Email + Password + OTP)
// ═══════════════════════════════════════════════════════════════════════════
router.post('/register', register)
router.post('/verify-otp', verifyOTPCode)  // ← NEW: Verify OTP
router.post('/resend-otp', resendVerification)  // Renamed from resend-verification
router.post('/login', login)
router.post('/refresh', refresh)
router.post('/logout', logout)
router.get('/profile', authenticate, getProfile)

// ═══════════════════════════════════════════════════════════════════════════
// PASSWORD MANAGEMENT
// ═══════════════════════════════════════════════════════════════════════════
router.get('/account-type', authenticate, getAccountType)      // Kiểm tra loại tài khoản
router.post('/change-password', authenticate, changePassword)  // Đổi mật khẩu (TH2, TH3)
router.post('/create-password', authenticate, createPassword)  // Tạo mật khẩu cho Google (TH1)

// ═══════════════════════════════════════════════════════════════════════════
// FORGOT PASSWORD (Public - không cần auth)
// ═══════════════════════════════════════════════════════════════════════════
router.post('/forgot-password', forgotPassword)      // Gửi OTP reset password
router.post('/verify-reset-otp', verifyResetOTP)     // Xác thực OTP reset
router.post('/reset-password', resetPassword)        // Đặt mật khẩu mới

// ═══════════════════════════════════════════════════════════════════════════
// GOOGLE OAUTH
// ═══════════════════════════════════════════════════════════════════════════
router.get(
  "/google",
  passport.authenticate("google", {
    scope: ["profile", "email"],
    session: false,
  })
);

router.get("/google/callback", (req, res, next) => {
  passport.authenticate("google", { session: false }, async (err, user, info) => {
    console.log("🔍 Google OAuth callback - user:", user?.email, "info:", info);

    // Xử lý lỗi authentication
    if (err) {
      console.error("❌ Google OAuth error:", err);
      return res.redirect(`${process.env.FRONTEND_URL}/?error=google_auth_failed`);
    }

    // Xử lý trường hợp user bị banned (từ passport strategy)
    if (!user && info?.message === "banned") {
      console.log("🚫 Banned user blocked from Google login (from strategy)");
      return res.redirect(`${process.env.FRONTEND_URL}/?error=account_banned`);
    }

    // Không có user (lỗi chung)
    if (!user) {
      console.log("❌ Google OAuth failed - no user returned, info:", info);
      return res.redirect(`${process.env.FRONTEND_URL}/?error=google_auth_failed`);
    }

    // Kiểm tra lại user có bị banned không (double check)
    const { data: profile } = await supabase.from("profiles").select("is_banned").eq("id", user.id).single();
    console.log("🔍 Profile check - is_banned:", profile?.is_banned);

    if (profile?.is_banned) {
      console.log(`🚫 Banned user blocked from Google login: ${user.email}`);
      return res.redirect(`${process.env.FRONTEND_URL}/?error=account_banned`);
    }

    // Tạo JWT token cho user
    const accessToken = jwt.sign({ userId: user.id, email: user.email }, JWT_SECRET, { expiresIn: "15m" });

    const refreshToken = jwt.sign({ userId: user.id }, JWT_SECRET, { expiresIn: "7d" });

    // Lưu refresh token vào cookie
    res.cookie("refreshToken", refreshToken, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax", // Dùng 'lax' thay vì 'strict' để hoạt động với OAuth redirect
      maxAge: 7 * 24 * 60 * 60 * 1000,
    });

    // Redirect về Frontend với access token
    res.redirect(`${process.env.FRONTEND_URL}/auth/callback?token=${accessToken}`);
  })(req, res, next);
});
//console.log("🔍 Passport instance in routes:", passport);
//console.log("📋 Available strategies in routes:", Object.keys(passport._strategies));

export default router;
